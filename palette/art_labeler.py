...
class ImageLabeler:
    ...
    def setup_controls(self):
        ...
        ttk.Label(self.controls, text="Image Type (1-regular, 2-artwork):").pack(anchor='w')
        ...
        self.material_combo = ttk.Combobox(self.controls, textvariable=self.material_var, values=materials)
        ...
        # Photo angle
        ttk.Label(self.controls, text="Photo Angle (3-straight, 4-angled):").pack(anchor='w')
        ...
        # Cropping quality
        ttk.Label(self.controls, text="Cropping (5-well, 6-includes surroundings):").pack(anchor='w')
        ...
        # Navigation buttons
        ttk.Button(self.controls, text="Previous (←)", command=self.prev_image).pack(pady=5)
        ttk.Button(self.controls, text="Next (→)", command=self.next_image).pack(pady=5)
        ttk.Button(self.controls, text="Save (s)", command=self.save_labels).pack(pady=5)
    ...
    def load_current_labels(self):
        ...
        if image_path in self.labels:
            ...
            self.cropping_var.set(label_data.get('cropping', CroppingQuality.UNKNOWN.value))
    ...
    def redraw_signature_boxes(self):
        ...
        for box in self.current_boxes:
            self.canvas.create_rectangle(
                ...
                outline='red', width=2, tags='signature_box'
            )
...
def main():
    ...
    parser.add_argument("--csv", type=str, help="Path to CSV file with image data")
    parser.add_argument(
        "-o",
        "--output",
        type=str,
